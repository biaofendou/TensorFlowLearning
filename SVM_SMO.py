...
plt.xlabel('Feature/Exam x')
plt.ylabel('Feature/Exam y')
...
class SVM:
    ...
    def score(self, X_test, y_test):
        ...
        for i in range(len(X_test)):
            ...
            if result == y_test[i]:
                right_count += 1
        return right_count / len(X_test), result_list
...
svm = SVM(max_iter=2000)
print(svm.fit(x, y))
score, y_pred = svm.score(x, y)
print(score)
w = svm._weight()
print(sk.metrics.f1_score(y, y_pred))
x_ = np.linspace(-6, 6)
plt.plot(x_, x_ * w[0] + w[1], color='g')
plt.show()
